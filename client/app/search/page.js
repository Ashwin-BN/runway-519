...
export default function SearchPage() {
  ...
  const handleMarkAsSold = async (itemId) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/items/${itemId}/sold`, {
        method: "PUT",
        headers: {
      ...
      if (res.ok) {
        setSelectedItem((current) =>
          current && current._id === itemId ? { ...current, status: "sold" } : current
        );
        setShowModal(false);
  ...
  const handleSetMarkdownPrice = async (itemId, price) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/items/${itemId}/markdown`, {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      if (res.ok) {
        setSelectedItem((current) =>
          current && current._id === itemId ? { ...current, markdownPrice: price } : current
        );
        setShowModal(false);
